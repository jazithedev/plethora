...
\ResponsiveFileManager::$aConfig = $aFileManagerConfig;

// CHANGES ADAPTED TO FRAMEWORK ===>
$_GET['linktofirework'] = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
define('FORCE_GET_PARAM', TRUE);
require_once __DIR__.'/../../index.php';
ob_clean();
// <=== CHANGES ADAPTED TO FRAMEWORK

$aFileManagerConfig = \ResponsiveFileManager::$aConfig;

return array_merge(
	$aFileManagerConfig,
	array(
		'MaxSizeUpload' => ((int)(ini_get('post_max_size')) < $aFileManagerConfig['MaxSizeUpload'])
			? (int)(ini_get('post_max_size')) : $aFileManagerConfig['MaxSizeUpload'],
		'ext'=> array_merge(
			$aFileManagerConfig['ext_img'],
			$aFileManagerConfig['ext_file'],
			$aFileManagerConfig['ext_misc'],
			$aFileManagerConfig['ext_video'],
			$aFileManagerConfig['ext_music']
		),
		...
		'aviary_defaults_config' => array(
			'apiKey'     => $aFileManagerConfig['aviary_apiKey'],
			'apiVersion' => 3,
			'language'   => 'en',
			'theme'      => 'light',
			'tools'      => 'all'
